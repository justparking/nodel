...
public class NanoHTTPD {
    ...
    private Response tryServeFromFolder(File dir, String uri, Properties header, boolean noFallback) {
		...
		try {
			...
			else if (HTTP_INTERNALERROR.equalsIgnoreCase(response.status))
				response = null;

			else if (HTTP_FORBIDDEN.equalsIgnoreCase(response.status))
				response = null;
     ...
     */
    public NanoHTTPD(int port, File wwwroot, boolean allowBrowsing) throws IOException {
        initLogging();
    ...
    /**
     * Starts handling requests. This will block until it's sure the socket is
     * ready to serve.
     */
    public void start() {
        final Flag abort = new Flag();
        ...
        this.myThread = new Thread(new Runnable() {
            public void run() {
                ...
                NanoHTTPD.this.logger.info("'socket.accept' thread has ended.");
            }
        });
        ...
        try {
            synchronized (abort) {
                // keep waiting until the abort flag is up or the socket is
                // bound
                while (!(abort.value || this.myServerSocket.isBound()))
                    abort.wait(500);
                
                if (abort.value)
                    return;
                
                NanoHTTPD.this.logger.info("HTTPD started.");
            } // (sync)
    ...
    public static void main(String[] args) {
        myOut.println("NanoHTTPD 1.26 (C) 2001,2005-2012 Jarno Elonen and (C) 2010 Konstantinos Togias\n" + "(Command line options: [-p port] [-d root-dir] [--licence])\n");
        ...
        try {
            new NanoHTTPD(port, wwwroot, true);
        } catch (IOException ioe) {
            ...
            try {
                ...
                if (method.equalsIgnoreCase("POST")) {
                    ...
                    if (contentType.equalsIgnoreCase("multipart/form-data")) {
                        ...
                        // pass the input stream to another handler
        ...
        private void decodeParms(String parms, Properties p) throws InterruptedException {
            ...
            while (st.hasMoreTokens()) {
                ...
                if (sep >= 0)
                    ...
                    // Just indicate the existence of this variable by using 'true'
                    // (map doesn't allow 'null' values)
                    p.put(decodePercent(e).trim(), Boolean.TRUE);
                }
            }
        }
     ...
     * instead of '+'.
     */
    protected static String encodeUri(String uri) {
        String newUri = "";
        ...
        if (!noFallback && res == null && f.isDirectory()) {
            ...
            if (res == null) {
                ...
                // No index file, list the directory if it is readable
                    ...
                    res = new Response(HTTP_FORBIDDEN, MIME_PLAINTEXT, "FORBIDDEN: No directory browsing permitted.");
                }
            }
        }
    ...
    protected Response prepareRedirectResponse(String uri) {
        Response res = new Response(HTTP_REDIRECT, MIME_HTML, "<html><body>Redirected: <a href=\"" + uri + "\">" + uri + "</a></body></html>");
